 ...
 * Time:    2020-05-31 10:12:46
 */

namespace CodeSinging\IndentTree;
...
class IndentTree
{
     ...
     * @return array
     */
    protected function toTree(array $data, int $parentId = 0)
    {
        $tree = [];
        foreach ($data as $item) {
            if ($item[$this->config['parentId']] == $parentId) {
                $children = $this->toTree($data, $item[$this->config['id']]);
                $children && $item[$this->config['children']] = $children;
                $tree[] = $item;
            }
        }
        return $tree;
     ...
     * @return array
     */
    protected function getIndent(array $treeData, string $path = '', int $depth = 0, array $parentMap = [])
    {
        $list = [];
     ...
     * @return array
     */
    protected function toIndent(array $treeData)
    {
     ...
     * @return array
     */
    public function indentData(array $data)
    {
